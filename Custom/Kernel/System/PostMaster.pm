...
sub GetEmailParams {
    ...
    # set dynamic fields
    $GetParam{'X-OTRS-DynamicField-Department'} = 'ГорячаяЛиния';
    $GetParam{'X-OTRS-DynamicField-ReceptionMethod'} = 'Адрес электронной почты';
    $GetParam{'X-OTRS-DynamicField-ContactInfo'} = $GetParam{'From'};

    # if mail was autogenerated(contains specific subject), parse table within and get params
    if ( index($GetParam{Subject}, decode('UTF-8', 'Вопрос клиента из оболочки КонсультантПлюс'), 0) != -1) {
        my $HTMLBody;
        for my $Attachment ( @{$GetParam{Attachment}} ) {
            if ( index($Attachment->{ContentType}, 'text/html', 0) != -1 ) {
                $HTMLBody = $Attachment->{Content};
            }
        }

        if ( length $HTMLBody ) {
            my $TableObject = HTML::TableExtract->new();
            $TableObject->parse( $HTMLBody );
        
            my %TicketParams;

            foreach my $Table ( $TableObject->tables() ) {
                foreach my $Row ( $Table->rows() ) {
                    ...
                    $Value =~ s/^\s+//;
                    $Value =~ s/\s+$//;
                    $TicketParams{$Key} = $Value;
                }
            }

            # because numero sign breaks everything
            my ($DistrKey) = grep /дистрибутива основной системы$/, keys %TicketParams;
            my ($Distr) = $TicketParams{$DistrKey} =~ /^(\d+_\d+)/;
            my $TO = `redis-cli get $Distr`;
            $TO =~ s/\v//g;

            # replace param fields with fields from table
            $GetParam{'X-OTRS-CustomerNo'} = $TO;
            $GetParam{'SenderEmailAddress'} = $TicketParams{'Электронный адрес'};
            $GetParam{'From'} = $TicketParams{'Электронный адрес'};
            $GetParam{'Return-Path'} = $TicketParams{'Электронный адрес'};
            $GetParam{'X-Sender'} = $TicketParams{'Электронный адрес'};
            $GetParam{'Body'} = "ФИО клиента: ".$TicketParams{'ФИО клиента'}.
                "\nТелефон: ".$TicketParams{'Телефон'}.
                "\nВопрос: \n".$TicketParams{'Вопрос'};
            $GetParam{'X-OTRS-DynamicField-ReceptionMethod'} = 'Кнопка в СПС КонсультантПлюс';
            $GetParam{'X-OTRS-DynamicField-ContactInfo'} = $TicketParams{'Электронный адрес'};
            
            # create customer user if no user found
            my $CustomerUserObject = $Kernel::OM->Get('Kernel::System::CustomerUser');
            my %List = $CustomerUserObject->CustomerSearch(
                PostMasterSearch => lc( $GetParam{From} )
            );
            my %CustomerData;
            ...
            if ( $CustomerData{UserLogin} ) {
                ...
                my $NewUserLogin = $CustomerUserObject->CustomerUserAdd(
                    Source => 'CustomerUser',
                    UserFirstname => '.',
                    UserLastname => $TicketParams{'ФИО клиента'},
                    UserCustomerID => $GetParam{'X-OTRS-CustomerNo'},
                    UserLogin => $TicketParams{'ФИО клиента'},
                    UserEmail => $TicketParams{'Электронный адрес'},
                    ValidID => 1,
                    UserID => $Self->{PostmasterUserID},
                );
                if ( $NewUserLogin ) {
                    $GetParam{'X-OTRS-CustomerUser'} = $NewUserLogin;
                }
            }
        }
    }
