...
sub new {
    ...
    $Self->{CustomerCompanyTable} = $Self->{CustomerUserMap}->{Params}->{CustomerCompanyTable}
        || die "Need CustomerUser->Params->CustomerCompanyTable in Kernel/Config.pm!";
...
sub CustomerSearch {
    ...
    $SQL .= " FROM $Self->{CustomerTable}, $Self->{CustomerCompanyTable} WHERE ($Self->{CustomerTable}.$Self->{CustomerID} = $Self->{CustomerCompanyTable}.$Self->{CustomerID}) AND ";
    if ( $Param{Search} ) {
