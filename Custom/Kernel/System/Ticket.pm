...
###########################################################################################################
    ...
    $Self->_SendUpdateRequest( TicketID => $TicketID );
###########################################################################################################
...
###########################################################################################################
    # send http request with updated info
    $Self->_SendUpdateRequest( TicketID => $Param{TicketID} );
###########################################################################################################
...
=item _SendUpdateRequest()

    Send http request with updated info

    my $Result = _SendUpdateRequest(
...
sub _SendUpdateRequest {
    ...
    my $url = "localhost";
    my %UserData = $Self->_GetRequestData( 
        TicketID => $Param{TicketID}, 
    );

    my $content = $Self->_HashToString(
        %UserData,
    );
    chop $content;

    my $response = `python /opt/otrs/Custom/Tasker.py '$content'`;

    #my $ua = LWP::UserAgent->new();
    #my $request  = HTTP::Request->new(POST => $url);
    #$request->header('content-type' => 'application/json');
    #$request->content(Encode::encode_utf8($content));
...
=item _GetRequestData()

    Returns hash with ticket fields and article fields to be sent via http

    my %Fields = _GetRequestData(
        TicketID = $TicketID,
    );
...
sub _GetRequestData {
    ...
    my %FirstArticle = $Self->ArticleFirstArticle(
        TicketID      => $TicketID,
        DynamicFields => 1,
    );
    ...
    my $UserObject = $Kernel::OM->Get('Kernel::System::User');
    my %Creator = $UserObject->GetUserData(
    ...
    my $Closed = 0;
    if ($Ticket{StateType} eq "closed") {$Closed = 1;}
    ...
    for my $i (0..$#Articles) {
        ...
        my %ArticleHash = %{$Articles[$i]};
        $i += 1;
        next ARTICLES if (
             ($ArticleHash{SenderType} eq 'system')
             or ($ArticleHash{SenderType} eq 'customer')
             or ($ArticleHash{DynamicField_LawType} eq 'Горячий вопрос')); #protection from one broken ticket
             #or ( index( $ArticleHash{ArticleType},'internal' ) > 0 ));
        if ( index( $ArticleHash{ArticleType}, 'internal' ) > 0 ) {
            $Ticket{DynamicField_Comment} .= " / " . $ArticleHash{Body};
            next ARTICLES;
        }
        if (($ArticleHash{ArticleType} eq 'email-external')
            and ($ArticleHash{SenderType} eq 'agent')
            and ($Closed eq 1)) {
            $ArticleHash{ReplySent} = 1;
        } else {
            $ArticleHash{ReplySent} = 0;
        }
        my %ArticleCreator = $UserObject->GetUserData(
            UserID => $ArticleHash{CreatedBy},
        );
        $ArticleHash{FromRealname} = "$ArticleCreator{UserLastname} $ArticleCreator{UserFirstname}";
        $TicketArticles{$ArticleKey} = {%ArticleHash};
    }
    ...
    my %RequestData = (
        ...
        CustomerUserName => "$CustomerUser{UserLastname} $CustomerUser{UserFirstname}",
        ReceptionMethod => $Ticket{DynamicField_ReceptionMethod},
        CustomerUserPhone => $CustomerUser{UserPhone} || '',
        CustomerUserEmail => $CustomerUser{UserEmail} || '',
        Type => $Ticket{Type},
        FirstArticleBody => $FirstArticle{Body},
        CreatedByID => $Ticket{CreateBy},
        CreatedByName => "$Creator{UserLastname} $Creator{UserFirstname}",
        Closed => $Closed,
        ResponsibleID => $Ticket{ResponsibleID},
        ResponsibleName => "$Responsible{UserLastname} $Responsible{UserFirstname}",
        Comment => $Ticket{DynamicField_Comment} || '',
        Age => $Ticket{Age},
        Feedback => $Ticket{DynamicField_TicketFeedback},
        ContactInfo => $Ticket{DynamicField_ContactInfo},
        Department => $Ticket{DynamicField_Department},
        NotifiedAt => $Ticket{DynamicField_NotifiedAt},
...
=item _HashToString()
    ...
    my $String = _HashToString(%Hash)
    
=cut

sub _HashToString {
    my ( $Self, %HashToConvert ) = @_;
    my $String = "{ ";

    ITER:
    while ( my($key, $value) = each %HashToConvert ) {
        $String .= "\"$key\":";
        ...
        if (ref($value) eq "HASH") {
            $String .= $Self->_HashToString( %{$value} );
        }
        elsif (ref($value) eq "ARRAY") {
            $String .= $Self->_ArrayToString( @{$value} );
        }
        else {
            $value =~ s/['"<>&]//g;
            $String .= "\"$value\",";
...
sub _ArrayToString {
    my ($Self, @ArrayToConvert) = @_;
    my $String = "[ ";

    ITER:
    for my $item (@ArrayToConvert) {
        if (ref($item) eq "HASH") {
            $String .= $Self->_HashToString( %{$item} );
        }
        elsif (ref($item) eq "ARRAY") {
           $String .= $Self->_ArrayToString( @{$item} );
        }
        else {
            $item =~ s/['"<>&]//g;
            $String .= "\"$item\",";
