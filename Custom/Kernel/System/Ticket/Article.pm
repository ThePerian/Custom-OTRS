...
sub ArticleSend {
    ...
    # if article is external, use body made to include dynamic fields
    my $Body = $Param{Body};
    my %ArticleTypes = $Self->ArticleTypeList( Result => 'HASH' );
    if ( index($ArticleTypes{$Param{ArticleTypeID}}, 'external') > -1 ) {
        if ( defined $Param{ReplyBody} ) {
            $Body = $Param{ReplyBody}
        }
    }
    ...
    # send mail
    my %ParamSwitchBody = %Param;
    $ParamSwitchBody{Body} = $Body;
