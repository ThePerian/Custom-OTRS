...
sub TicketSearch {
    ...
    my $CustomerCompanyDynamicFields = [];
	...
	# Check all configured customer company dynamic fields
        $CustomerCompanyDynamicFields = $DynamicFieldObject->DynamicFieldListGet(
...
##################################################################################################################################################
    my $CustomerCompanyJoinSQL = '';#$Self->_CustomerCompanyIndexQuerySQL( Data => \%Param ) || '';

    # sql, use also customer company table if needed
    $SQLFrom .= $CustomerCompanyJoinSQL;
##################################################################################################################################################
    ...
    # add customer ids if we have customer name
    if ( $Param{CustomerCompanyName} ) {
        my $CustomerCompanyObject = $Kernel::OM->Get('Kernel::System::CustomerCompany');

        my %CustomerCompanyList = $CustomerCompanyObject->CustomerCompanyList(
            Search => "*$Param{CustomerCompanyName}*",
        );

        $SQLExt .= " AND st.customer_id IN (" . join(', ', keys(%CustomerCompanyList)) . ") ";
    }
    ...
    DYNAMIC_FIELD:
    for my $DynamicField ( @{$TicketDynamicFields}, @{$ArticleDynamicFields}, @{$CustomerCompanyDynamicFields} ) {
	    ...
	    elsif ( $DynamicField->{ObjectType} eq 'CustomerCompany' ) {
                if ( !$CustomerCompanyJoinSQL ) {
                    $CustomerCompanyJoinSQL = ' INNER JOIN customer_company cc ON st.customer_id = cc.customer_id ';
                    $SQLFrom .= $CustomerCompanyJoinSQL;
                }

                $SQLFrom .= "INNER JOIN dynamic_field_value dfv$DynamicFieldJoinCounter
                    ON (cc.customer_id = dfv$DynamicFieldJoinCounter.object_id
                        AND dfv$DynamicFieldJoinCounter.field_id = " .
                    $DBObject->Quote( $DynamicField->{ID}, 'Integer' ) . ") ";
        ...
        for my $Count ( 0 .. $#SortByArray ) {
            ...
            if ( $ValidDynamicFieldParams{ $SortByArray[$Count] } ) {
                ...
                my $DynamicField = $TicketDynamicFieldName2Config{$DynamicFieldName} ||
                    ...
                    $CustomerCompanyDynamicFieldName2Config{$DynamicFieldName};
                ...
                if ( !$DynamicFieldJoinTables{$DynamicFieldName} ) {
                    ...
                    elsif ( $CustomerCompanyDynamicFieldName2Config{$DynamicFieldName} ) {
                        if ( !$CustomerCompanyJoinSQL ) {
                            $CustomerCompanyJoinSQL = ' INNER JOIN customer_company cc ON st.customer_id = cc.customer_id ';
                            $SQLFrom .= $ArticleJoinSQL;
                        }
