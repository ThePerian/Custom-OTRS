...
sub Run {
    ...
    elsif ( $Self->{Subaction} eq 'MarkAsImportant' ) {
...
sub _ArticleMenu {
    ...
    # check if free text should be shown
    if (
        $ConfigObject->Get('Frontend::Module')->{AgentArticleFreeText}
        )
    {
        push @MenuItems, {
            ItemType    => 'Link',
            Description => Translatable('Edit free text'),
            Name        => Translatable('Free text'),
            Class       => 'AsPopup PopupType_TicketAction',
            Link =>
                "Action=AgentArticleFreeText;TicketID=$Ticket{TicketID};ArticleID=$Article{ArticleID}"
        };
    }
