...
sub new {
    ...
    $Self->{DynamicField} = $Kernel::OM->Get('Kernel::System::DynamicField')->DynamicFieldListGet(
        ...
        FieldFilter => $ConfigObject->Get("Frontend::Admin::$Self->{Action}")->{DynamicField} || {},
    );
...
sub Run {
	...
	#get dynamic fields
	    ...
	    $GetParam{DynamicFields}->{ $DynamicFieldConfig->{Name} } = 
...
=pod
    # get default dynamic field values
    ...
    for my $DynamicFieldConfig ( @{ $Self->{DynamicField} } ) {
        ...
        $DynamicFieldDefaults{ $DynamicFieldConfig->{Name} } = $DefaultValue;
    }
=cut
    # get HTML lines for dynamic fields
    my %DynamicFieldHTML;
    my $DynamicFieldBackendObject = $Kernel::OM->Get('Kernel::System::DynamicField::Backend');
    my $ParamObject = $Kernel::OM->Get('Kernel::System::Web::Request');
	...
	# get value stored on the database or default value
	#if ( $Param{Action} eq 'Change' ) {
	...
	#    $Value = $DynamicFieldDefaults{ $DynamicFieldConfig->{Name} } || '';
	#}
    ...
    for my $DynamicFieldConfig ( @{ $Self->{DynamicField} } ) {
        ...
        my $DynamicFieldHTMLCurrent = $DynamicFieldHTML{ $DynamicFieldConfig->{Name} };

        $LayoutObject->Block(
            Name => 'DynamicField',
            Data => {
                Name  => $DynamicFieldConfig->{Name},
                Label => $DynamicFieldHTMLCurrent->{Label},
                Field => $DynamicFieldHTMLCurrent->{Field},
            },
        );
