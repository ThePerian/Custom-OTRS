...
perform specific functions after the Value set for this object type.
    ...
    my $Success = $DynamicFieldTicketHandlerObject->PostValueSet(
        ...
        ObjectID           => $ObjectID,                # ID of the current object that the field
                                                        # must be linked to, e. g. CustomerID
...
sub PostValueSet {
    ...
    return if !$DBObject->Do(
        SQL => 'UPDATE customer_company SET change_time = current_timestamp, '
            . ' change_by = ? WHERE id = ?',
...
#    $TicketObject->_TicketCacheClear( TicketID => $Param{ObjectID} );
    ...
    # Trigger event.
    $CustomerCompanyObject->EventHandler(
        Event => 'CustomerCompanyDynamicFieldUpdate_' . $Param{DynamicFieldConfig}->{Name},
        Data  => {
            FieldName => $Param{DynamicFieldConfig}->{Name},
            Value     => $Param{Value},
